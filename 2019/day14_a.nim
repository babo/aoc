...
proc makeFuel(recipe: Table[string, (int, seq[(string, int)])]): int =
  var stocks = initTable[string, int]()

  proc getPart(name: string, quant: int): int =
    if name == "ORE":
      return quant
    ...
    var sum = 0
    while stocks.getOrDefault(name, 0) < quant:
      for p in ingredients:
        sum += getPart(p[0], p[1])
      stocks[name] = stocks.getOrDefault(name, 0) + produce

    stocks[name] = stocks.getOrDefault(name, 0) - quant
    return sum

  let rtv = getPart("FUEL", 1)
  echo rtv
  return rtv

proc allParts(input: string): int = makeFuel(readReactions(input))
...
"""
assert allParts(test5) == 2210736

echo allParts(readInput("day14.data"))
