...
impl Neighbors<Dim3> for Dim3 {
    fn neighbors(c: &Dim3) -> Vec<Dim3> {
        let mut rtv = Vec::new();
...
impl Neighbors<Dim4> for Dim4 {
    fn neighbors(c: &Dim4) -> Vec<Dim4> {
        let mut rtv = Vec::new();
...
fn cycle<T: Neighbors<T> + Copy + Eq + std::hash::Hash>(input: &HashSet<T>) -> HashSet<T> {
    let mut hs = HashSet::<T>::new();

    for cube in input {
        let nb = T::neighbors(cube);
