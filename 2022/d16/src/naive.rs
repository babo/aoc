...
impl<'a> Volcano<'a> {
    ...
    fn take_action(&self, state: &EvalState, visited_pressure: &mut [i32; 1680]) -> Vec<EvalState> {
        let mut next = Vec::new();
        ...
        let visited = visited_pressure[max_pos];

        if current_pressure >= visited - 57 {
            visited_pressure[max_pos] = current_pressure;
            ...
            println!(
                "Arrive {name} at {current_time} with {current_pressure} but visited at {}",
                visited,
            );
        }
        next
...
pub fn best_route(input: &str) -> usize {
    ...
    let mut visited_pressure = [-1i32; 1680];

    while !current_states.is_empty() {
        future_states.clear();
        current_states.iter().for_each(|state| {
            v.take_action(state, &mut visited_pressure)
                .iter()
                .for_each(|next_state| {
    ...
    println!("{:?}", visited_pressure.iter().max());
